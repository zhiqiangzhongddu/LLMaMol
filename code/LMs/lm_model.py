...
class BertRegressor(PreTrainedModel):
    def __init__(self, model, dropout=0.0, seed=0, cla_bias=True, feat_shrink=''):
        super().__init__(model.config)
        self.bert_encoder = model
        self.dropout = nn.Dropout(dropout)
        self.feat_shrink = feat_shrink
    ...
    def forward(
        ...
        if self.feat_shrink:
            cls_token_emb = self.feat_shrink_layer(cls_token_emb)
        logits = self.regressor(cls_token_emb)
...
class BertRegInfModel(PreTrainedModel):
    ...
    def forward(
        ...
        # Extract outputs from the model
        bert_outputs = self.bert_regressor.bert_encoder(
            input_ids=input_ids,
            attention_mask=attention_mask,
            return_dict=return_dict,
            output_hidden_states=True
        ...
        cls_token_emb = emb.permute(1, 0, 2)[0]
        if self.feat_shrink:
            cls_token_emb = self.bert_regressor.feat_shrink_layer(
                cls_token_emb)
        logits = self.bert_regressor.regressor(cls_token_emb)
